...
testinfra_hosts = ['docker://minimal_common_container']

@pytest.fixture(scope="module", autouse=True)
def container(client, image):
    container = client.containers.run(
        image.id,
        name="minimal_common_container",
        detach=True,
        tty=True,
        command="bash"
    )
...
# 
# TODO: No python, check not exists
#def test_python3_executable(host):
#    assert host.exists("python3")

def test_microdnf_package_installed(host):
    assert host.package("microdnf").is_installed  
